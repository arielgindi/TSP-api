...
namespace RouteOptimizationApi
{
    public static class TspAlgorithm
    {
        ...
        public static List<Delivery> GenerateRandomDeliveries(int count, int minCoord, int maxCoord)
        {
            ...
            var deliveries = new List<Delivery>(count);
            var usedCoords = new HashSet<(int, int)> { (Depot.X, Depot.Y) };
        ...
        {
            var pending = new List<Delivery>(allDeliveries ?? Enumerable.Empty<Delivery>());
            var route = new List<Delivery> { Depot };
            var current = Depot;
            if (!pending.Any()) { route.Add(Depot); return route; }
        ...
        public static List<Delivery> ConstructClarkeWrightRoute(List<Delivery> allDeliveries)
        {
            ...
            var representativeToRoute = new Dictionary<int, List<int>>();
            var deliveryToRep = new Dictionary<int, int>();
            var firstDelInRoute = new Dictionary<int, int>();
            var lastDelInRoute = new Dictionary<int, int>();
            ...
            {
                int repI = deliveryToRep[s.DeliveryIdI], repJ = deliveryToRep[s.DeliveryIdJ];
                if (repI == repJ) continue;
                bool canMerge = lastDelInRoute[repI] == s.DeliveryIdI && firstDelInRoute[repJ] == s.DeliveryIdJ;
                if (canMerge)
                {
                    var routeI = representativeToRoute[repI];
                    var routeJ = representativeToRoute[repJ];
            ...
            var finalRoute = new List<Delivery> { Depot };
            if (representativeToRoute.Count > 1)
            {
                foreach (var kvp in representativeToRoute.Values)
                    kvp.ForEach(dId => finalRoute.Add(allDeliveries.First(d => d.Id == dId)));
            }
            else
            {
                var singleList = representativeToRoute.Values.First();
                singleList.ForEach(dId => finalRoute.Add(allDeliveries.First(d => d.Id == dId)));
            }
        ...
        public static List<Delivery> OptimizeRouteUsing2Opt(List<Delivery> initialRoute)
        {
            ...
            var currentRoute = new List<Delivery>(initialRoute);
            bool improvement = true;
            int maxIterations = 10000, iter = 0;

            while (improvement && iter < maxIterations)
            {
                improvement = false; iter++;
                for (int i = 0; i < currentRoute.Count - 3; i++)
                {
                    for (int j = i + 2; j < currentRoute.Count - 1; j++)
                    {
                        var A = currentRoute[i];
                        var B = currentRoute[i + 1];
                        var C = currentRoute[j];
                        var D = currentRoute[j + 1];
        ...
        {
            minMakespan = double.MaxValue; bestCuts = null; long iterations = 0;
            if (optimizedRoute == null || optimizedRoute.Count < 2)
            {
                minMakespan = 0; bestCuts = Array.Empty<int>();
            ...
            {
                minMakespan = 0; double maxSingle = 0; var cuts = new List<int>();
            ...
            for (int i = 1; i <= deliveryCount; i++)
                lowerBound = Math.Max(lowerBound, ComputeSubRouteDistanceWithDepot(optimizedRoute, i, i));

            double upperBound = ComputeTotalRouteDistance(optimizedRoute), optimal = upperBound;
            ...
            while (lowerBound <= upperBound && iterations < maxBinIters)
            {
                ...
                {
                    optimal = mid; currentBestCuts = potentialCuts; upperBound = mid - Constants.Epsilon;
                }
                else lowerBound = mid + Constants.Epsilon;
                reporter?.Invoke(iterations);
            }
            minMakespan = optimal;
        ...
        {
            cuts = null;
            if (route == null || route.Count < 2) { cuts = Array.Empty<int>(); return true; }
            int n = route.Count - 2, driversUsed = 1, start = 1;
        ...
        public static double CalculateEuclideanDistance(Delivery d1, Delivery d2)
        {
            ...
            double dx = d2.X - d1.X, dy = d2.Y - d1.Y;
